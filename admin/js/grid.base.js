...
$.fn.jqGrid = function( p ) {
	...
	$.fn.setGridWidth = function(nwidth, shrink) {
		return this.each(function(){
			...
			var initwidth = 0, tblwidth, brd=5, l, vc=0, isSafari,cw,hs=false, scw=5, aw;
			if (!$t.grid ) {return;}
	...
	return this.each( function() {
		...
		var addCell = function(t,row,cell,pos,irow, srvr) {
			var td;
			td = $("<td></td>");
		...
		var formatter = function (elem, row, cellval , colpos, rwdat, _act){
		...
		var setPager = function (){
			...
			str, pgcnt, lft, cent, rgt, twd, tdw, 
			...
			tdw = $("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).width();
			$("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).append(pgl).width(500);
			twd = $("td#"+pgid+"_"+ts.p.pagerpos+ " > table.ui-pg-table","#"+pgcnt).width();
			if(twd > 0) {
				$("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).width(twd+13);
			} else {
		...
		var setColWidth = function () {
			var initwidth = 0, tblwidth, brd=5, l, vc=0,scw=5,cw,hs=false,aw;
			...
			if(ts.p.shrinkToFit===true) {
				...
				} else if(!hs && grid.width-(initwidth+brd*vc) !== 0) {
					cr = grid.width-(initwidth+brd*vc)-scw;
				}
		...
		var arf = ts.p.hidegrid===true ? $("<a href='#'/>").addClass('ui-jqgrid-titlebar-close').hover(
